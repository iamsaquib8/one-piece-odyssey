...
Opening an arc is a title card, not a document: the island panorama with a saga-colour wash, the arc name slammed in, a hook line, chapter counters, and a ship crossing once. Below it the reader is staged like a manga volume — who is aboard, who stands where (crew gold, allies teal, wild cards violet, foes coral), then the voyage as a gold route the reader draws by scrolling. Each beat is a diorama: the staged location under a beat-specific camera, mood (dawn, day, dusk, night, storm, flashback = sepia, grain and film sprockets), and the people present standing on the shore. Battles open with a versus card and a verdict stamp, then play as a scene: the fight stands on the arc's island, the side that throws the blow lunges while the other reels, an impact star bursts with a comic sound word, speed lines rake outward, and a momentum marker swings until the verdict settles it. The blow-by-blow is derived from the captions and the recorded sides, so no fight needs choreography written by hand. Everything stays code-native pixel art; no manga panels are reproduced.
...
## Reader discovery
A shared paper control strip exposes completed-arc progress and the Character journeys, Mystery board and Logbook routes. Keep the existing navigation hierarchy. Journeys use a deep-sea masthead, paper character cards and gold route markers with 44px targets. Mysteries use a violet masthead and native paper disclosures; coral and sea accents identify open and resolved threads, with dark text for small labels. Both views reflow at 320px. Limited mode uses neutral portrait silhouettes and short bounded profile text. Never imply a hidden clue or future character through an empty state.
