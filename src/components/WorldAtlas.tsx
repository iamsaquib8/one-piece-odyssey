...
import {ArrowLeft,ArrowRight,Bookmark,BookmarkCheck,BookOpen,Check,ChevronLeft,ChevronRight,Compass,Flag,Focus,Layers,MapPin,Maximize2,Minimize2,Minus,Pause,Play,Plus,Search,Ship,Wind,X} from 'lucide-react';
...
interface Props {reader:ReaderState;onOpen:(kind:NonNullable<Route['kind']>,id:string)=>void;onNavigate:(view:View)=>void;onSave:(id:string)=>void;route:Route}
...
export default function WorldAtlas({reader,onOpen,onNavigate,onSave,route}:Props){
  ...
  const effectiveMotion=reader.motion&&document.documentElement.dataset.motion==='on'&&visible&&!route.kind;
  ...
  useEffect(()=>{if(!effectiveMotion)setPlaying(false);},[effectiveMotion]);
  useEffect(()=>{
    if(!playing)return;
  ...
  function moveTo(next:Camera,animate=true){
    ...
    if(!animate||!effectiveMotion){setCamera(target);return;}
  ...
  function selectPlace(p:AtlasPlace,focus=true){setSelected(p.id);setTab('place');setHovered(null);if(focus){moveTo({x:p.x,y:p.y,zoom:mode==='globe'?1.7:Math.max(2.3,cameraRef.current.zoom)});setFocusToken(t=>t+1);}}
  ...
  return <section className="atlas-page" aria-labelledby="world-title">
    ...
    <div ref={panelRef} className={`atlas-shell ${expanded?'atlas-expanded':''}`} data-moving={dragging||playing}>
      <div className="atlas-toolbar"><div className="atlas-title"><Compass size={21}/><strong>WORLD ATLAS</strong><span>VOL. 01</span></div><div className="atlas-projection" role="group" aria-label="Map projection"><button className={mode==='globe'?'active':''} aria-pressed={mode==='globe'} onClick={()=>{setMode('globe');setVisible(true);}}>3D globe</button><button className={mode==='chart'?'active':''} aria-pressed={mode==='chart'} onClick={()=>setMode('chart')}>Flat chart</button></div><nav aria-label="Map regions">{regionCameras.map(r=><button key={r.id} className={region===r.id?'active':''} aria-pressed={region===r.id} onClick={()=>{setRegion(r.id);setPlaying(false);setResetToken(t=>t+1);moveTo(mode==='globe'?{...r.camera,zoom:1}:r.camera);}}>{r.label}</button>)}</nav><button className="atlas-icon" aria-label={expanded?'Exit expanded map':'Expand map'} onClick={()=>setExpanded(x=>!x)}>{expanded?<Minimize2 size={18}/>:<Maximize2 size={18}/>}</button></div>
      <div className="atlas-body"><div className="atlas-canvas-column">
        <div className="atlas-canvas-wrap" data-ambient={effectiveMotion?'on':'off'}>
          {mode==='globe'?<Suspense fallback={<div className="globe-loading" role="status"><Compass/> Building the globe…</div>}><GlobeView selected={selected} onSelect={p=>selectPlace(p,false)} zoom={camera.zoom} region={region} focusToken={focusToken} resetToken={resetToken} motion={effectiveMotion} layers={layers} onFallback={()=>setMode('chart')}/></Suspense>:<svg ref={svgRef} className={`atlas-canvas ${dragging?'is-dragging':''}`} viewBox={`${camera.x-w/2} ${camera.y-h/2} ${w} ${h}`} tabIndex={0} role="group" aria-label="Interactive world map. Arrow keys to pan, plus and minus to zoom, zero to reset." onKeyDown={keyDown} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onPointerLeave={e=>{if(!e.currentTarget.hasPointerCapture(e.pointerId))pointerUp(e);}}>
            ...
            {visiblePlaces.map(p=>{
              ...
              const label=active||hover||layers.labels&&(p.major||zoom>=2.5);
          ...
          <div className="atlas-map-caption"><span className="map-live-dot"/> SCHEMATIC CHART <span>·</span> {atlasPlaces.length} PLACES</div>
          <div className="atlas-floating-controls"><button ref={layerButton} className={`atlas-icon ${layerMenu?'active':''}`} aria-label="Map layers" aria-expanded={layerMenu} onClick={()=>setLayerMenu(x=>!x)}><Layers size={18}/></button><span className="control-divider"/><button className="atlas-icon" aria-label="Zoom in" disabled={camera.zoom>=3.99} onClick={()=>moveTo({...cameraRef.current,zoom:Math.round((cameraRef.current.zoom+.5)*2)/2},false)}><Plus size={20}/></button><span className="atlas-zoom">{Math.round(zoom*100)}%</span><button className="atlas-icon" aria-label="Zoom out" disabled={camera.zoom<=1.01} onClick={()=>moveTo({...cameraRef.current,zoom:Math.round((cameraRef.current.zoom-.5)*2)/2},false)}><Minus size={20}/></button><span className="control-divider"/><button className="atlas-icon" aria-label="Reset view" onClick={()=>{setPlaying(false);setRegion('all');setResetToken(t=>t+1);moveTo(HOME_CAMERA,false);}}><Focus size={18}/></button></div>
          {layerMenu?<div className="atlas-layers" onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();setLayerMenu(false);layerButton.current?.focus();}}}><div><strong>Chart layers</strong><button aria-label="Close map layers" className="atlas-icon" onClick={()=>setLayerMenu(false)}><X size={15}/></button></div>{([['voyage','Voyage route'],['geography','Physical connections'],['story','Story connections'],['labels','Island labels']] as const).map(([key,label])=><label key={key}><span className={`layer-swatch ${key}`}/><span>{label}</span><input type="checkbox" checked={layers[key]} onChange={()=>setLayers(v=>({...v,[key]:!v[key]}))}/></label>)}</div>:null}
          {mode==='chart'?<button className="atlas-minimap" aria-label="Reset to whole world" onClick={()=>{setRegion('all');moveTo(HOME_CAMERA);}}><svg viewBox="0 0 240 140" aria-hidden="true"><rect width="240" height="140" fill="#103847"/><path d="M59 0v140M177 0v140" stroke="#a5644a" strokeWidth="9"/><path d="M62 74h178" stroke="#388391" strokeWidth="50" opacity=".35"/>{atlasPlaces.filter(p=>p.major).map(p=><circle key={p.id} cx={p.x/10} cy={p.y/10} r="1.5" fill="#e4cc90"/>)}<rect x={(camera.x-w/2)/10} y={(camera.y-h/2)/10} width={w/10} height={h/10} fill="#f7d680" fillOpacity=".09" stroke="#f7d680" strokeWidth="2"/></svg><span>WORLD OVERVIEW <Focus size={10}/></span></button>:null}
          <span className="atlas-map-hint">{mode==='globe'?'Drag to orbit · pinch or scroll to zoom':'Drag to explore · pinch to zoom · Ctrl/⌘ + scroll'}</span>
        </div>
        <div className="atlas-voyage-player"><div className="voyage-player-controls"><button className="atlas-icon" aria-label="Previous voyage stop" disabled={step===0} onClick={()=>{setPlaying(false);chooseStop(step-1);}}><ChevronLeft size={20}/></button><button className="atlas-icon play-voyage" aria-label={playing?'Pause voyage':'Play voyage'} disabled={!effectiveMotion} onClick={()=>{if(!playing){chooseStop(step===voyageStops.length-1?0:step);}setPlaying(x=>!x);}}>{playing?<Pause size={19}/>:<Play size={19}/>}</button><button className="atlas-icon" aria-label="Next voyage stop" disabled={step===voyageStops.length-1} onClick={()=>{setPlaying(false);chooseStop(step+1);}}><ChevronRight size={20}/></button></div><div className="voyage-scrubber"><div><span><Ship size={13}/>{stop.arc.chapters[0]>=435?'THOUSAND SUNNY':'GOING MERRY'}<b>·</b><strong>{stop.arc.name}</strong></span><span>{step+1} / {voyageStops.length}</span></div><input aria-label="Voyage chapter stop" type="range" min="0" max={voyageStops.length-1} step="1" value={step} onChange={e=>{setPlaying(false);chooseStop(Number(e.target.value));}}/><div className="scrubber-labels"><span>ROMANCE DAWN</span><span>{chapterLabel(stop.arc)}</span><span>ELBAF ARRIVAL</span></div></div><button className="atlas-read-stop" onClick={()=>onOpen('arc',stop.arc.id)}><BookOpen size={18}/><span>Read arc</span><ArrowRight size={15}/></button></div>
      </div>
