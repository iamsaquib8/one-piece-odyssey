...
import { crewCoverageThrough, crewCategories, crewFirstAppearance, type CrewCategory } from '../data/crews';
...
function storyMatches(crewArcIds: string[], filter: StoryFilter, arcIdsBySaga: Map<string, Set<string>>) {
...
function MemberPortrait({ id, query, onOpen, characterById }: { id: string; query: string; onOpen: CrewViewProps['onOpen']; characterById: Map<string, ReturnType<typeof useReaderCatalog>['characters'][number]> }) {
...
export default function CrewView({ reader, onOpen, onSave }: CrewViewProps) {
  ...
  const { arcs, characters, crews } = catalog;
  ...
  const visibleSagas = useMemo(() => sagas.filter((saga) => arcs.some((arc) => arc.sagaId === saga.id)), [arcs]);
  const arcIdsBySaga = useMemo(() => new Map(visibleSagas.map((saga) => [saga.id, new Set(arcs.filter((arc) => arc.sagaId === saga.id).map((arc) => arc.id))])), [arcs, visibleSagas]);
  ...
  const visibleCrews = useMemo(() => {
    ...
    return crews.filter((crew) => {
      ...
      if (!storyMatches(crew.arcIds, story, arcIdsBySaga)) return false;
    ...
    }).slice().sort((a, b) => horizon===null&&sort === 'relevance'
      ? b.relevance - a.relevance
      : (crewFirstAppearance[a.id] ?? Number.MAX_SAFE_INTEGER) - (crewFirstAppearance[b.id] ?? Number.MAX_SAFE_INTEGER));
  }, [arcById, arcIdsBySaga, category, characterById, crews, deferredQuery, horizon, sort, story]);
  ...
  return <section className="explorer fleet-view" aria-labelledby="crew-title">
    <header className="fleet-hero">
      <div className="fleet-hero-copy">
        ...
        <p>{horizon===null?<>From one borrowed hat to the powers contesting the final sea: {crews.length} crews and factions, ordered by their weight in the story. Relevance describes narrative reach, not strength. Profiles cover through chapter {crewCoverageThrough}.</>:<>Crews and factions revealed through your completed arcs. Records grow as you move the reading horizon.</>}</p>
      </div>
      <div className="fleet-compass" aria-hidden="true">
        <Compass />
        <span>{horizon===null?crewCoverageThrough:horizon}</span>
      ...
      <dl className="fleet-stats">
        ...
        <div><dt>{horizon===null?7:crews.length}</dt><dd>{horizon===null?'Grand Fleet ships':'Visible groups'}</dd></div>
      </dl>
    </header>
    ...
    <div className="fleet-tools" aria-label="Crew filters">
      ...
      <label className="fleet-select"><span>Story point</span><select value={story} onChange={handleStory} aria-label="Filter by saga or arc relevance">
        ...
        {visibleSagas.map((saga) => <optgroup key={saga.id} label={saga.name}>
      ...
      <label className="fleet-select"><span>Order</span><select value={horizon===null?sort:'appearance'} onChange={(event) => setSort(event.target.value as Sort)} aria-label="Sort crews">
        {horizon===null?<option value="relevance">Story relevance</option>:null}
        <option value="appearance">First appearance</option>
      </select></label>
    </div>

    {horizon===null?<div className="fleet-categories" aria-label="Filter by relationship">
    ...
    {visibleCrews.length ? <div className="fleet-grid">
      {visibleCrews.map((crew, index) => {
        ...
        return <article className="fleet-card" key={crew.id} style={{ '--crew-accent': crew.color, animationDelay: `${Math.min(index * 35, 350)}ms` } as CSSProperties}>
          {horizon===null?<div className="fleet-card-topline"><span>{crew.category}</span><span>R.{String(crew.relevance).padStart(3, '0')}</span></div>:null}
          <button className="fleet-card-heading" onClick={() => onOpen('crew', crew.id)}>
            <CharacterPortrait characterId={captain?.id??''} name={captain?.name??'Unknown captain'} color={captain?.color ?? crew.color} className="captain-portrait" />
            <span><small>{captain?.role ?? 'Crew record'}</small><strong><Highlight query={query}>{crew.name}</Highlight></strong>{captain?<em>{crew.category==='factions'?'Featured: ':'Led by '}<Highlight query={query}>{captain.name}</Highlight></em>:null}</span>
            <ArrowRight size={20} />
          </button>
          <p>{crew.description}</p>
          {crew.ship ? <div className="fleet-ship"><ShipWheel size={16} /><span><small>Vessel</small>{crew.ship}</span></div> : null}
          {members.length ? <div className="fleet-members"><span className="micro">KEY MEMBERS</span><div>{members.map((id) => <MemberPortrait key={id} id={id} query={query} onOpen={onOpen} characterById={characterById} />)}</div></div> : null}
    ...
    </div> : <div className="fleet-empty"><Users size={42} /><h2>{crews.length?'No crew crosses those coordinates.':'No crews in your reading horizon yet.'}</h2><p>{crews.length?'Try another story point or a shorter name.':'Complete your first arc to begin revealing the crews that cross the voyage.'}</p><button className="button" onClick={() => { setQuery(''); setCategory('all'); setStory('all'); }}>{crews.length?'Reset the atlas':'Clear filters'}</button></div>}
  </section>;
}
